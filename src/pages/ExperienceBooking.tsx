...
const ExperienceBooking = () => {
  ...
  // Availability-confirmed state from detail page
  const availabilityConfirmed = location.state?.availabilityConfirmed || false;
  const actualTotal = location.state?.actualTotal || null;
  const bookingRef = location.state?.bookingRef || null;
  const passedTravelDate = location.state?.travelDate || "";
  const passedPaxCounts = location.state?.paxCounts || {};
  const passedOptionCode = location.state?.selectedOptionCode || "";

  const initialTravelers = location.state?.travelers || Object.values(passedPaxCounts).reduce((a: number, b: any) => a + (Number(b) || 0), 0) || 2;
  const [form, setForm] = useState({
    ...
    preferredDate: passedTravelDate,
    selectedOption: passedOptionCode,
    notes: "",
  });

  const [productCode, setProductCode] = useState<string | undefined>(rawProductCode);

  // Resolve slug to productCode if needed
  useEffect(() => {
    if (rawProductCode) { setProductCode(rawProductCode); return; }
    if (!slug) return;
    const resolve = async () => {
      try {
        const { data } = await supabase.functions.invoke("unified-tour-search", {
          body: { action: "resolve-slug", slug },
        });
        if (data) hydrateTourDataFromWire(data);
        if (data?.success) setProductCode(data.productCode);
        else { toast.error("Tour not found"); setLoading(false); }
      } catch { toast.error("Failed to resolve tour"); setLoading(false); }
    };
    resolve();
  }, [slug, rawProductCode]);
    ...
    <Layout>
      <div className="bg-hero-gradient py-8">
        <div className="container mx-auto px-4">
          <Link to={slug ? `/tours/${(product.destination?.name || 'experience').trim().toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-')}/${slug}` : `/tours/experience/${productCode}`} className="text-primary-foreground/70 hover:text-primary-foreground text-sm mb-2 inline-block">← Back to Tour</Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-6">
            ...
            <Button type="submit" size="lg" className="w-full" disabled={submitting}>
              {submitting ? "Processing..." : availabilityConfirmed ? `Confirm & Pay ${formatPrice(total)}` : `Submit Booking ${formatPrice(total)}`}
            </Button>
          </form>
