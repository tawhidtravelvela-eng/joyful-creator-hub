...
import { useCurrency, CURRENCIES, getSymbol } from "@/contexts/CurrencyContext";
...
const statusColors: Record<string, string> = {
  ...
  "Needs Payment": "bg-warning/50/10 text-warning border border-warning/15",
  "Awaiting Payment": "bg-warning/50/10 text-warning border border-warning/15",
};
...
type ActivePage = "dashboard" | "all-bookings" | "flight-bookings" | "hotel-bookings" | "tour-bookings" | "favourites" | "profile" | "support" | "requests" | "wallet" | "b2b-access" | "affiliate";

const Dashboard = () => {
  const { user, loading: authLoading } = useAuth();
  const navigate = useNavigate();
  const { formatPrice, formatBookedPrice, formatFromSource, formatDirectPrice, convertFromSource, currency } = useCurrency();
  ...
  const [walletByCurrency, setWalletByCurrency] = useState<Record<string, number>>({});
  const [affiliateData, setAffiliateData] = useState<{ id: string; affiliate_code: string; status: string; total_earnings: number; wallet_balance: number; commission_rate: number } | null>(null);
  ...
  const fetchData = async (userId: string) => {
    ...
    try {
      const [bookingsRes, profileRes, walletRes] = await Promise.all([
        ...
        supabase.from("wallet_transactions" as any).select("amount, type, status, currency").eq("user_id", userId).eq("status", "completed"),
      ]);
      ...
      // Aggregate wallet balance per currency (transactions may be in different currencies)
      ...
      setWalletByCurrency(byCurrency);

      // Fetch affiliate data
      const { data: affData } = await supabase.from("affiliates").select("id, affiliate_code, status, total_earnings, wallet_balance, commission_rate").eq("user_id", userId).maybeSingle();
      setAffiliateData(affData as any);
    } catch (err) {
  ...
  // Convert each booking from its booked currency to display currency for an accurate cross-currency total
  const totalSpentInDisplay = bookings
    .filter(b => b.status !== "Cancelled")
    .reduce((sum, b) => sum + convertFromSource(Number(b.total) || 0, (b.booked_currency || "USD").toUpperCase()), 0);
  // Wallet balance converted to display currency (sum across multiple currencies)
  const walletBalanceInDisplay = Object.entries(walletByCurrency).reduce(
    (sum, [cur, amt]) => sum + convertFromSource(amt, cur),
    0
  );
  ...
  const navItems = [
    ...
    { id: "affiliate", label: "Affiliate", icon: Share2, page: "affiliate" as ActivePage },
  ...
  const statsCards = [
    ...
    },
    {
      ...
      value: formatDirectPrice(walletBalanceInDisplay),
      icon: Wallet,
      color: "text-primary",
      bg: "bg-primary/10",
    ...
    {
      label: "Total Spent",
      value: formatDirectPrice(totalSpentInDisplay),
      icon: CreditCard,
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-t border-border">
        <div className="rounded-xl bg-gradient-to-br from-primary/5 to-primary/10 p-4">
          ...
          <p className="text-xl font-bold text-foreground">{formatDirectPrice(walletBalanceInDisplay)}</p>
        </div>
      </div>
    </div>
  );
      ...
      <>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4 mb-6">
          {statsCards.map((stat, i) => (
            ...
            >
              <Card
                className={`hover:shadow-md transition-shadow border-border/50 ${(stat as any).onClick ? "cursor-pointer" : ""}`}
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="mb-6">
          <Card className="border-border/50 bg-gradient-to-r from-primary/5 to-transparent">
            <CardContent className="p-4 md:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  ...
                  {affiliateData && affiliateData.status === "approved" ? (
                    <>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Code: <span className="font-mono font-medium text-foreground">{affiliateData.affiliate_code}</span> · {affiliateData.commission_rate}% commission · Balance: {formatPrice(affiliateData.wallet_balance)}
                      </p>
                    </>
                  ) : affiliateData && affiliateData.status === "pending" ? (
                    <>
                      <p className="text-sm font-semibold text-foreground">Affiliate Application Pending</p>
                      <p className="text-xs text-muted-foreground">Your application is under review.</p>
                    </>
                  ) : (
                    <>
                      <p className="text-sm font-semibold text-foreground">Earn with Our Affiliate Program</p>
                      <p className="text-xs text-muted-foreground">Share your referral link and earn commissions on every booking.</p>
                    </>
                  )}
                </div>
              </div>
              <Button
                size="sm"
                variant={affiliateData?.status === "approved" ? "default" : "outline"}
                className="gap-1.5 text-xs"
                onClick={() => navigate("/affiliate")}
              >
                {affiliateData?.status === "approved" ? "Open Dashboard" : "Learn More"} <ArrowRight className="h-3.5 w-3.5" />
              </Button>
            </CardContent>
          </Card>
        </motion.div>
                    ...
                    return (
                      <div key={b.id} className="flex items-center gap-4 px-5 py-4 hover:bg-muted/30 transition-colors">
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className="text-sm font-semibold text-foreground">{formatBookedPrice(b.total, b.booked_currency || "USD")}</p>
                        ...
                        <div className="flex gap-1 flex-shrink-0">
                          ...
                          {b.status === "Needs Payment" && (
                            <Button size="sm" variant="default" className="h-8 text-xs bg-warning/50 hover:bg-warning" title="Pay Now" onClick={() => navigate(`/booking/pay/${b.id}`)}>
                      ...
                      {filtered.map((b) => {
                        ...
                        return (
                          <TableRow key={b.id} className="hover:bg-muted/20">
                            ...
                            <TableCell className="font-semibold text-sm">{formatBookedPrice(b.total, b.booked_currency || "USD")}</TableCell>
                            ...
                            <TableCell className="text-right">
                              <div className="flex gap-1 justify-end">
                                ...
                                {b.status === "Needs Payment" && (
                                  <Button size="sm" variant="default" className="h-8 text-xs bg-warning/50 hover:bg-warning" onClick={() => navigate(`/booking/pay/${b.id}`)}>
                    ...
                    return (
                      <div key={b.id} className="p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="text-right flex-shrink-0">
                            <p className="text-sm font-bold text-foreground">{formatBookedPrice(b.total, b.booked_currency || "USD")}</p>
                            <div className="flex gap-1 mt-1 justify-end">
                              <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => downloadInvoice(b)}>
                                <Download className="h-3.5 w-3.5" />
                              </Button>
                              {b.status === "Needs Payment" && (
                                <Button size="sm" variant="default" className="h-7 text-[10px] bg-warning/50 hover:bg-warning px-2" onClick={() => navigate(`/booking/pay/${b.id}`)}>
                                  Pay
                                </Button>
                              )}
                              {b.status !== "Cancelled" && (
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between py-2 border-b border-border">
              ...
              <span className="text-sm font-semibold text-foreground">{formatDirectPrice(totalSpentInDisplay)}</span>
            </div>
  ...
  const renderMainContent = () => {
    switch (activePage) {
      ...
      case "wallet": return <WalletSection userId={user!.id} balance={walletBalanceInDisplay} onBalanceChange={refreshData} />;
      case "b2b-access": return <B2BAccessRequests />;
      case "affiliate": {
        // Redirect to full affiliate dashboard
        navigate("/affiliate");
        return null;
      }
