...
/**
 * Status palette — kept as semantic green/amber/red/blue regardless of
 * tenant brand. Industry convention: travelers expect green = confirmed,
 * red = cancelled. Brand-tinting status would harm legibility and trust.
 */
...
/* ------------------------------------------------------------------ *
 *  Brand color helpers — convert the tenant's primary hex into the    *
 *  full set of derived shades used across the e-ticket (header        *
 *  gradient, accent backgrounds, table headers, secondary text).      *
 *  Pure JS so values are baked into inline style at render time —     *
 *  print/PDF rendering stays deterministic.                           *
 * ------------------------------------------------------------------ */
function hexToHsl(hex: string): { h: number; s: number; l: number } {
  const m = hex.replace("#", "").match(/.{1,2}/g);
  if (!m) return { h: 210, s: 100, l: 50 };
...
function shadeOf(hex: string, lDelta: number): string {
  const { h, s, l } = hexToHsl(hex);
  return hslToHex(h, s, Math.max(0, Math.min(100, l + lDelta)));
}
function tintOf(hex: string, alphaPct: number): string {
  // Soft tinted background — mix brand color with white at given strength.
  const { h, s } = hexToHsl(hex);
  return hslToHex(h, Math.min(100, s), 100 - alphaPct);
}
...
const ETicket = () => {
  ...
  /**
   * Brand-driven palette. The tenant's primary brand color (set in
   * Studio → Brand) drives the e-ticket's header gradient, table
   * header rows, accent backgrounds, and link colors. Fallback to the
   * platform default blue when no primary is configured.
   *
   * Status colors (Confirmed / Cancelled / Pending) intentionally stay
   * semantic green/red/amber — see comment on `statusBg`.
   */
  const BRAND_PRIMARY = (branding.primary_color || "#0092ff").trim();
  const PRIMARY = BRAND_PRIMARY;
  const PRIMARY_DARK = shadeOf(BRAND_PRIMARY, -14);
  const PRIMARY_LIGHT = shadeOf(BRAND_PRIMARY, +6);
  const ACCENT_BG = tintOf(BRAND_PRIMARY, 92);
  const GOLD = "#b45309";
  const BORDER = "#e2e8f0";
  const TEXT = "#0f172a";
  const TEXT2 = "#475569";
        ...
        <div style={{ background: "white", borderRadius: 12, overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.08)", position: "relative" }} className="print:!shadow-none print:!rounded-none">
          ...
          {isFlight && (cd.passengers && cd.passengers.length > 0 ? (
            <div style={{ padding: "20px 36px", borderBottom: `1px solid ${BORDER}` }}>
              <SectionHeader brandTint={ACCENT_BG} brandColor={PRIMARY} icon={<Users style={{ width: 14, height: 14 }} />} title="Passenger Details" />
          ...
          {isFlight && (departure || arrival) && (
            <div style={{ padding: "20px 36px", borderBottom: `1px solid ${BORDER}` }}>
              <SectionHeader brandTint={ACCENT_BG} brandColor={PRIMARY} icon={<Plane style={{ width: 14, height: 14 }} />} title="Flight Itinerary" />
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 12, marginTop: 10, border: `1px solid ${BORDER}`, borderRadius: 8, overflow: "hidden" }}>
                <thead>
                  <tr style={{ background: PRIMARY }}>
          ...
          {!isFlight && (
            <div style={{ padding: "20px 36px", borderBottom: `1px solid ${BORDER}` }}>
              <SectionHeader brandTint={ACCENT_BG} brandColor={PRIMARY} icon={<TypeIcon style={{ width: 14, height: 14 }} />} title={isHotel ? "Reservation Details" : "Tour Details"} />
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 12, marginTop: 10, border: `1px solid ${BORDER}`, borderRadius: 8, overflow: "hidden" }}>
                <tbody>
                  {details
          ...
          {cd.ancillaries && (cd.ancillaries as any[]).length > 0 && (
            <div style={{ padding: "20px 36px", borderBottom: `1px solid ${BORDER}` }}>
              <SectionHeader brandTint={ACCENT_BG} brandColor={PRIMARY} icon={<Armchair style={{ width: 14, height: 14 }} />} title="Add-on Services" />
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 12, marginTop: 10, border: `1px solid ${BORDER}`, borderRadius: 8, overflow: "hidden" }}>
                <thead>
                  <tr style={{ background: PRIMARY }}>
          ...
          {/* ═══════ FARE SUMMARY ═══════ */}
          <div style={{ padding: "20px 36px", borderBottom: `1px solid ${BORDER}` }}>
            <SectionHeader brandTint={ACCENT_BG} brandColor={PRIMARY} icon={<CheckCircle2 style={{ width: 14, height: 14 }} />} title="Payment Summary" />
            <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0, fontSize: 12, marginTop: 10, border: `1px solid ${BORDER}`, borderRadius: 8, overflow: "hidden" }}>
              <tbody>
