...
import { Loader2, Save, Eye, EyeOff, Plane, TestTube, Globe, Database, DollarSign, Receipt, RefreshCw, Building2, MapPin, Settings2, ShieldCheck, Lock, Car } from "lucide-react";
import VaultCredentialFields from "@/components/admin/VaultCredentialFields";
...
const AdminApiSettings = () => {
  ...
  const [tpSettings, setTpSettings] = useState<TravelportSettings>({
    target_branch: "", pcc: "", username: "", password: "",
  ...
  // Amadeus Transfer state
  const [amtData, setAmtData] = useState<ApiProvider | null>(null);
  const [amtActive, setAmtActive] = useState(false);
  const [amtEnv, setAmtEnv] = useState("test");

  // Booking119 Taxi state
  const [b119Data, setB119Data] = useState<ApiProvider | null>(null);
  const [b119Active, setB119Active] = useState(false);
  ...
  const [tjhEnv, setTjhEnv] = useState("production");
  ...
  // Sabre state
  const [sabreData, setSabreData] = useState<ApiProvider | null>(null);
  const [sabreActive, setSabreActive] = useState(false);
  const [sabreStudentFare, setSabreStudentFare] = useState(false);
  const [sabreEnv, setSabreEnv] = useState("test");

  // Viator state
  const [viatorData, setViatorData] = useState<ApiProvider | null>(null);
  const [viatorActive, setViatorActive] = useState(false);
  const [viatorEnv, setViatorEnv] = useState("production");

  // Hotelston Hotel state
  const [hotelstonData, setHotelstonData] = useState<ApiProvider | null>(null);
  const [hotelstonActive, setHotelstonActive] = useState(false);
  const [hotelstonEnv, setHotelstonEnv] = useState("test");
  ...
  const [apiSourceCurrencies, setApiSourceCurrencies] = useState<Record<string, string>>({
    travelport: "BDT", amadeus: "USD", local_inventory: "USD", tripjack: "INR",
  });
    ...
    try {
      ...
      if (data) {
        ...
        if (tp) {
          ...
          setTpEnv(env);
          setTpSettings({
            target_branch: s.target_branch || "", pcc: (s as any).pcc || "", username: s.username || "", password: s.password || "",
        ...
        const amt = data.find((d: any) => d.provider === "amadeus_transfer");
        if (amt) {
          setAmtData(amt as any);
          setAmtActive(amt.is_active);
          const s = amt.settings as any;
          setAmtEnv(s?.environment || "test");
        }
        const b119 = data.find((d: any) => d.provider === "booking119_taxi");
        if (b119) {
          setB119Data(b119 as any);
          setB119Active(b119.is_active);
        }
        ...
        const viator = data.find((d: any) => d.provider === "viator");
        if (viator) {
          setViatorData(viator as any);
          setViatorActive(viator.is_active);
          const vs = viator.settings as any;
          setViatorEnv(vs?.environment || "production");
        }
        const hotelston = data.find((d: any) => d.provider === "hotelston_hotel");
        if (hotelston) {
          setHotelstonData(hotelston as any);
          setHotelstonActive(hotelston.is_active);
          const hs = hotelston.settings as any;
          setHotelstonEnv(hs?.environment || "test");
        }
        const tjh = data.find((d: any) => d.provider === "tripjack_hotel");
        if (tjh) {
          setTjhData(tjh as any);
          setTjhActive(tjh.is_active);
          const tjhSettings = tjh.settings as any;
          setTjhEnv(tjhSettings?.environment || "test");
        }
        ...
        const sabre = data.find((d: any) => d.provider === "sabre");
        if (sabre) {
          setSabreData(sabre as any);
          setSabreActive(sabre.is_active);
          const sabreSettings = sabre.settings as any;
          setSabreEnv(sabreSettings?.environment || "test");
          setSabreStudentFare(sabreSettings?.student_fare_enabled ?? false);
        }
  ...
  const handleSaveAmadeusTransfer = async () => {
    setSaving("amadeus_transfer");
    try {
      const payload = { environment: amtEnv } as any;
      if (amtData) {
        const { error } = await supabase.from("api_settings").update({ settings: payload, is_active: amtActive }).eq("id", amtData.id);
        if (error) throw error;
      } else {
        const { data: inserted, error } = await supabase.from("api_settings").insert({ provider: "amadeus_transfer", settings: payload, is_active: amtActive }).select().single();
        if (error) throw error;
        if (inserted) setAmtData(inserted as any);
      }
      toast({ title: "Settings saved", description: "Amadeus Transfer API settings updated." });
    } catch (err: any) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
    } finally { setSaving(null); }
  };

  const handleTestAmadeusTransfer = async () => {
    setTesting("amadeus_transfer");
    try {
      const { data, error } = await supabase.functions.invoke("amadeus-transfer", {
        body: { action: "search", origin: { iataCode: "SIN" }, destination: { addressLine: "Marina Bay Sands", cityName: "Singapore" }, transferDate: "2026-05-01", transferTime: "10:00", passengers: 2 },
      });
      if (error) throw error;
      if (data?.success) toast({ title: "Connection successful", description: `Amadeus Transfer API working — ${data.offers?.length || 0} offers found.` });
  ...
  // Viator handlers
  const handleSaveViator = async () => {
    setSaving("viator");
    try {
      const payload = { environment: viatorEnv };
      if (viatorData) {
        const { error } = await supabase.from("api_settings").update({ settings: payload as any, is_active: viatorActive }).eq("id", viatorData.id);
        if (error) throw error;
      } else {
        const { data: row, error } = await supabase.from("api_settings").insert({ provider: "viator", settings: payload as any, is_active: viatorActive }).select("*").single();
        if (error) throw error;
        setViatorData(row as any);
      }
      toast({ title: "Settings saved", description: "Viator settings updated successfully." });
    } catch (err: any) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
    } finally { setSaving(null); }
  };

  const handleTestViator = async () => {
    setTesting("viator");
    try {
      const { data, error } = await supabase.functions.invoke("unified-tour-search", {
        body: { action: "freetext", searchText: "paris", sandbox: viatorEnv === "sandbox" },
      });
      if (data) hydrateTourDataFromWire(data);
      if (error) throw error;
      if (data?.success !== false) toast({ title: "Connection successful", description: "Viator API is reachable." });
      else toast({ title: "Connection failed", description: data?.error || "Could not connect.", variant: "destructive" });
    } catch (err: any) {
      toast({ title: "Test failed", description: err.message, variant: "destructive" });
    } finally { setTesting(null); }
  ...
  const handleTestTripjackHotel = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("unified-hotel-search", { body: { action: "test" } });
  ...
  const handleSaveBooking119Taxi = async () => {
    setSaving("booking119_taxi");
    try {
      const payload = {} as any;
      if (b119Data) {
        const { error } = await supabase.from("api_settings").update({ settings: payload, is_active: b119Active }).eq("id", b119Data.id);
        if (error) throw error;
      } else {
        const { data: inserted, error } = await supabase.from("api_settings").insert({ provider: "booking119_taxi", settings: payload, is_active: b119Active }).select().single();
        if (error) throw error;
        if (inserted) setB119Data(inserted as any);
      }
      toast({ title: "Settings saved", description: "Booking119 Taxi settings updated." });
    } catch (err: any) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
    } finally { setSaving(null); }
  };

  const handleTestBooking119Taxi = async () => {
    setTesting("booking119_taxi");
    try {
      const { data, error } = await supabase.functions.invoke("booking119-taxi", {
        body: { action: "search", pickupAirport: "SIN", dropoffAddress: "Marina Bay Sands, Singapore", startDateTime: "2026-05-15T14:00:00", passengers: 2 },
      });
      if (error) throw error;
      if (data?.success) toast({ title: "Connection successful", description: `Booking119 Taxi API working — ${data.offers?.length || 0} offers found.` });
      else toast({ title: "Connection failed", description: data?.error || "Could not connect.", variant: "destructive" });
    } catch (err: any) {
      toast({ title: "Test failed", description: err.message, variant: "destructive" });
    } finally { setTesting(null); }
  ...
  const handleSyncTripjackHotels = async () => {
    setSyncing(true);
    setSyncProgress("Starting hotel sync...");
    try {
      const { data, error } = await supabase.functions.invoke("unified-hotel-search", {
        body: { action: "sync-hotels", maxPages: 500 },
      });
      if (data) hydrateHotelDataFromWire(data);
      if (error) throw error;
      if (data?.success) {
        setSyncProgress(`Synced ${data.totalSynced} hotels (${data.pages} pages). ${data.complete ? "Complete!" : `More remaining — next cursor available.`}`);
        toast({ title: "Hotel sync complete", description: `${data.totalSynced} hotels synced across ${data.pages} pages.` });
      } else {
  ...
  // Sabre handlers
  const handleSaveSabre = async () => {
    setSaving("sabre");
    try {
      const payload = { environment: sabreEnv, student_fare_enabled: sabreStudentFare };
      if (sabreData) {
        const { error } = await supabase.from("api_settings").update({ settings: payload as any, is_active: sabreActive }).eq("id", sabreData.id);
        if (error) throw error;
      } else {
        const { data: row, error } = await supabase.from("api_settings").insert({ provider: "sabre", settings: payload as any, is_active: sabreActive }).select("*").single();
        if (error) throw error;
        setSabreData(row as any);
      }
      toast({ title: "Settings saved", description: "Sabre settings updated successfully." });
    } catch (err: any) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
    } finally { setSaving(null); }
  };

  const handleTestSabre = async () => {
    setTesting("sabre");
    try {
      const { data, error } = await supabase.functions.invoke("sabre-search", { body: { test: true } });
      if (error) throw error;
      if (data?.success) toast({ title: "Connection successful", description: "Sabre API authentication successful." });
      else toast({ title: "Connection failed", description: data?.error || "Could not authenticate.", variant: "destructive" });
    } catch (err: any) {
      toast({ title: "Test failed", description: err.message, variant: "destructive" });
    } finally { setTesting(null); }
  ...
  // Hotelston handlers
  const handleSaveHotelston = async () => {
    setSaving("hotelston_hotel");
    try {
      const payload = { environment: hotelstonEnv };
      if (hotelstonData) {
        const { error } = await supabase.from("api_settings").update({ settings: payload as any, is_active: hotelstonActive }).eq("id", hotelstonData.id);
        if (error) throw error;
      } else {
        const { data: row, error } = await supabase.from("api_settings").insert({ provider: "hotelston_hotel", settings: payload as any, is_active: hotelstonActive }).select("*").single();
        if (error) throw error;
        setHotelstonData(row as any);
      }
      toast({ title: "Settings saved", description: "Hotelston settings updated successfully." });
    } catch (err: any) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
    } finally { setSaving(null); }
  };

  const handleTestHotelston = async () => {
    setTesting("hotelston_hotel");
    try {
      const { data, error } = await supabase.functions.invoke("hotelston-hotel-search", { body: { test: true } });
      if (error) throw error;
      if (data?.success) toast({ title: "Connection successful", description: data.message || "Hotelston API is reachable." });
      else toast({ title: "Connection failed", description: data?.error || "Could not connect.", variant: "destructive" });
    } catch (err: any) {
      toast({ title: "Test failed", description: err.message, variant: "destructive" });
    } finally { setTesting(null); }
  ...
  const flightCount = [tpActive, amActive, invActive, tjfActive, sabreActive].filter(Boolean).length;
  const hotelCount = [tjhActive, hotelstonActive].filter(Boolean).length;
  const tourCount = [viatorActive].filter(Boolean).length;
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <Card className="border-border/60">
              ...
              <CardContent className="space-y-4 pt-0">
                <VaultCredentialFields
                  provider="travelport"
                  title="Credentials (Vault-encrypted)"
                  fields={[
                    { key: "username", label: "Username", placeholder: "Universal API username" },
                    { key: "password", label: "Password", type: "password", placeholder: "Universal API password" },
                    { key: "pcc", label: "PCC (Target Branch)", placeholder: "e.g. 2G8A" },
                    { key: "pp_username", label: "Pre-Prod Username", placeholder: "Pre-production username" },
                    { key: "pp_password", label: "Pre-Prod Password", type: "password", placeholder: "Pre-production password" },
                    { key: "pp_pcc", label: "Pre-Prod PCC", placeholder: "Pre-production PCC" },
                  ]}
                />
            ...
            <Card className="border-border/60">
              ...
              <CardContent className="space-y-4 pt-0">
                ...
                <VaultCredentialFields
                  ...
                  title="Credentials (Vault-encrypted)"
                  fields={[
                    { key: "api_key", label: "API Key", placeholder: "Amadeus API Key" },
                    { key: "api_secret", label: "API Secret", type: "password", placeholder: "Amadeus API Secret" },
                  ]}
                />
                <div className="flex items-center gap-2 pt-1">
            ...
            <Card className="border-border/60">
              ...
              <CardContent className="space-y-4 pt-0">
                ...
                <VaultCredentialFields
                  ...
                  title="Credentials (Vault-encrypted)"
                  fields={[
                    { key: "api_key", label: "API Key", type: "password", placeholder: "Tripjack API key" },
                    { key: "proxy_url", label: "Proxy URL", placeholder: "https://your-proxy.com (optional)" },
                    { key: "proxy_key", label: "Proxy Secret Key", type: "password", placeholder: "Proxy authentication key" },
                  ]}
                />
                <div className="flex items-center gap-2 pt-1">
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-sm">Sabre (Bargain Finder Max)</CardTitle>
                      <CardDescription className="text-xs">GDS flight search, pricing & booking</CardDescription>
                    </div>
                  </div>
                  <Switch checked={sabreActive} onCheckedChange={setSabreActive} />
                </div>
              </CardHeader>
              <CardContent className="space-y-4 pt-0">
                <div className="space-y-2">
                  <Label className="text-xs">Environment</Label>
                  <div className="flex gap-2">
                    <Button variant={sabreEnv === "test" ? "default" : "outline"} size="sm" className="text-xs h-8" onClick={() => setSabreEnv("test")}>Certification (Test)</Button>
                    <Button variant={sabreEnv === "production" ? "default" : "outline"} size="sm" className="text-xs h-8" onClick={() => setSabreEnv("production")}>Production</Button>
                  </div>
                  <p className="text-[11px] text-muted-foreground">Test uses cert.havail.sabre.com. Production uses api.havail.sabre.com.</p>
                </div>
                <div className="flex items-center justify-between rounded-lg bg-muted/40 px-3 py-2.5">
                  <div>
                    <p className="text-xs font-medium">Student Fare</p>
                  ...
                  <Switch checked={sabreStudentFare} onCheckedChange={setSabreStudentFare} />
                </div>
                <VaultCredentialFields
                  provider="sabre"
                  title="Credentials (Vault-encrypted)"
                  fields={[
                    { key: "client_id", label: "Client ID", placeholder: "Sabre Client ID" },
                    { key: "client_secret", label: "Client Secret", type: "password", placeholder: "Sabre Client Secret" },
                    { key: "pcc", label: "PCC", placeholder: "Pseudo City Code" },
                  ]}
                />
                <div className="flex items-center gap-2 pt-1">
                  <Button size="sm" onClick={handleSaveSabre} disabled={saving === "sabre"}>
                    {saving === "sabre" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Save className="w-3.5 h-3.5 mr-1.5" />}
                    Save
                  </Button>
                  <Button size="sm" variant="outline" onClick={handleTestSabre} disabled={testing === "sabre"}>
                    {testing === "sabre" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <TestTube className="w-3.5 h-3.5 mr-1.5" />}
                    Test
                  </Button>
                </div>
              </CardContent>
          ...
          <TabsContent value="hotels" className="space-y-5 mt-5">
            ...
            <Card className="border-border/60">
              ...
              <CardContent className="space-y-4 pt-0">
                <div className="space-y-2">
                  <Label className="text-xs">Environment</Label>
                  <Select value={tjhEnv} onValueChange={setTjhEnv}>
                    <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="test">Test (apitest.tripjack.com)</SelectItem>
                      <SelectItem value="production">Production (api.tripjack.com)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <VaultCredentialFields
                  provider="tripjack_hotel"
                  title="Credentials (Vault-encrypted)"
                  fields={[
                    { key: "api_key", label: "API Key", type: "password", placeholder: "Tripjack Hotel API key" },
                    { key: "proxy_url", label: "Proxy URL", placeholder: "https://your-proxy.com (optional)" },
                    { key: "proxy_key", label: "Proxy Secret Key", type: "password", placeholder: "Proxy authentication key" },
                  ]}
                />
                <div className="flex items-center gap-2 pt-1">
                  ...
                  <Button size="sm" variant="secondary" onClick={handleSyncTripjackHotels} disabled={syncing}>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-sm">Hotelston API v2</CardTitle>
                      <CardDescription className="text-xs">SOAP-based hotel search, availability & booking (EUR)</CardDescription>
                    </div>
                  </div>
                  <Switch checked={hotelstonActive} onCheckedChange={setHotelstonActive} />
                </div>
              </CardHeader>
              <CardContent className="space-y-4 pt-0">
                <div className="space-y-2">
                  <Label className="text-xs">Environment</Label>
                  <div className="flex gap-2">
                    <Button variant={hotelstonEnv === "test" ? "default" : "outline"} size="sm" className="text-xs h-8" onClick={() => setHotelstonEnv("test")}>Test (dev.hotelston.com)</Button>
                    <Button variant={hotelstonEnv === "production" ? "default" : "outline"} size="sm" className="text-xs h-8" onClick={() => setHotelstonEnv("production")}>Production</Button>
                  </div>
                </div>
                <VaultCredentialFields
                  provider="hotelston"
                  title="Credentials (Vault-encrypted)"
                  fields={[
                    { key: "email", label: "Email", placeholder: "Hotelston account email" },
                    { key: "password", label: "Password", type: "password", placeholder: "Hotelston account password" },
                  ]}
                />
                <div className="flex items-center gap-2 pt-1">
                  <Button size="sm" onClick={handleSaveHotelston} disabled={saving === "hotelston_hotel"}>
                    {saving === "hotelston_hotel" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Save className="w-3.5 h-3.5 mr-1.5" />}
                    Save
                  </Button>
                  <Button size="sm" variant="outline" onClick={handleTestHotelston} disabled={testing === "hotelston_hotel"}>
                    {testing === "hotelston_hotel" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <TestTube className="w-3.5 h-3.5 mr-1.5" />}
                    Test
                  </Button>
                </div>
              </CardContent>
            ...
            <div className="rounded-xl border-2 border-dashed border-border/50 p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground/70 mt-1">Booking.com and more</p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="tours" className="space-y-5 mt-5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge variant="secondary" className="text-[10px]">{tourCount} active</Badge>
            </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-sm">Viator</CardTitle>
                      <CardDescription className="text-xs">Search & book tours, activities, and experiences</CardDescription>
                    </div>
                  </div>
                  <Switch checked={viatorActive} onCheckedChange={setViatorActive} />
                </div>
              </CardHeader>
              <CardContent className="space-y-4 pt-0">
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    <Label className="text-xs">Environment</Label>
                    <Select value={viatorEnv} onValueChange={setViatorEnv}>
                      <SelectTrigger className="h-9 text-sm">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <VaultCredentialFields
                    ...
                    title="Credentials (Vault-encrypted)"
                    fields={[
                      { key: "api_key", label: "API Key", type: "password", placeholder: "Viator API key" },
                    ]}
                  />
                </div>
                <div className="flex items-center gap-2 pt-1">
                  <Button size="sm" onClick={handleSaveViator} disabled={saving === "viator"}>
                    {saving === "viator" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Save className="w-3.5 h-3.5 mr-1.5" />}
                    Save
                  </Button>
                  <Button size="sm" variant="outline" onClick={handleTestViator} disabled={testing === "viator"}>
                    {testing === "viator" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <TestTube className="w-3.5 h-3.5 mr-1.5" />}
                    Test
                  </Button>
                </div>
              </CardContent>
            ...
            {/* Placeholder for future tour APIs */}
            <div className="rounded-xl border-2 border-dashed border-border/50 p-8 text-center">
              <MapPin className="w-8 h-8 text-muted-foreground/40 mx-auto mb-3" />
              <p className="text-sm font-medium text-muted-foreground">More tour integrations coming soon</p>
              <p className="text-xs text-muted-foreground/70 mt-1">GetYourGuide, Musement, and more</p>
            </div>
          </TabsContent>

          {/* ═══════ TRANSFERS TAB ═══════ */}
          <TabsContent value="transfers" className="space-y-5 mt-5">
            <div className="flex items-center gap-2 mb-1">
              <Car className="w-4 h-4 text-primary" />
              <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">Transfer API Providers</h3>
            </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-sm">Amadeus Transfer API</CardTitle>
                      <CardDescription className="text-xs">Airport transfers — uses same Amadeus credentials</CardDescription>
                    </div>
                  </div>
                  <Switch checked={amtActive} onCheckedChange={setAmtActive} />
                </div>
              </CardHeader>
              <CardContent className="space-y-4 pt-0">
                <div className="space-y-2">
                  <Label className="text-xs">Environment</Label>
                  <div className="flex gap-2">
                    <Button variant={amtEnv === "test" ? "default" : "outline"} size="sm" className="text-xs h-8" onClick={() => setAmtEnv("test")}>Test</Button>
                    <Button variant={amtEnv === "production" ? "default" : "outline"} size="sm" className="text-xs h-8" onClick={() => setAmtEnv("production")}>Production</Button>
                  </div>
                  <p className="text-[11px] text-muted-foreground">Uses Amadeus Transfer Offers API. Credentials shared with Amadeus flight API (Vault-encrypted).</p>
                </div>
                <VaultCredentialFields
                  provider="amadeus_transfer"
                  title="Transfer-specific credentials (optional — falls back to Amadeus flight credentials)"
                  fields={[
                    { key: "api_key", label: "API Key (override)", placeholder: "Leave empty to use flight API key" },
                    { key: "api_secret", label: "API Secret (override)", type: "password", placeholder: "Leave empty to use flight API secret" },
                  ]}
                />
                <div className="flex items-center gap-2 pt-1">
                  <Button size="sm" onClick={handleSaveAmadeusTransfer} disabled={saving === "amadeus_transfer"}>
                    {saving === "amadeus_transfer" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Save className="w-3.5 h-3.5 mr-1.5" />}
                    Save
                  </Button>
                  <Button size="sm" variant="outline" onClick={handleTestAmadeusTransfer} disabled={testing === "amadeus_transfer"}>
                    {testing === "amadeus_transfer" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <TestTube className="w-3.5 h-3.5 mr-1.5" />}
                    Test
                  </Button>
                </div>
              </CardContent>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-sm">Booking119 Taxi API</CardTitle>
                      <CardDescription className="text-xs">Airport transfers via RapidAPI — live taxi quotes worldwide</CardDescription>
                    </div>
                  </div>
                  <Switch checked={b119Active} onCheckedChange={setB119Active} />
                </div>
              </CardHeader>
              <CardContent className="space-y-4 pt-0">
                <p className="text-[11px] text-muted-foreground">
                  Uses the Booking119 RapidAPI for taxi/transfer quotes. Requires a RAPIDAPI_KEY (set in project secrets or vault).
                  Endpoints: searchLocation → searchTaxi. Resolves IATA codes and addresses to Google Place IDs automatically.
                </p>
                <VaultCredentialFields
                  provider="booking119_taxi"
                  title="RapidAPI Key (optional — falls back to RAPIDAPI_KEY env secret)"
                  fields={[
                    { key: "rapidapi_key", label: "RapidAPI Key (override)", type: "password", placeholder: "Leave empty to use project RAPIDAPI_KEY secret" },
                  ]}
                />
                <div className="flex items-center gap-2 pt-1">
                  <Button size="sm" onClick={handleSaveBooking119Taxi} disabled={saving === "booking119_taxi"}>
                    {saving === "booking119_taxi" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Save className="w-3.5 h-3.5 mr-1.5" />}
                    Save
                  </Button>
                  <Button size="sm" variant="outline" onClick={handleTestBooking119Taxi} disabled={testing === "booking119_taxi"}>
                    {testing === "booking119_taxi" ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <TestTube className="w-3.5 h-3.5 mr-1.5" />}
                    Test
                  </Button>
                </div>
              </CardContent>
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-1.5">
                    {[
                      ...
                      { key: "tripjack", label: "Tripjack", icon: Plane },
                      { key: "amadeus", label: "Amadeus", icon: Globe },
                      { key: "sabre", label: "Sabre", icon: Plane },
