...
import { Save, Loader2, Plus, Pencil, Trash2, Globe, ExternalLink, UserPlus, X, ShieldCheck, Network, Settings2, Key, Copy, Eye, EyeOff, Cloud, RefreshCw, Mail, Upload } from "lucide-react";
...
const emptyTenant = {
  ...
  allowed_products: ["flights", "hotels", "tours"] as string[],
  settings: {
    site_name: "",
    logo_url: "",
    favicon_url: "",
...
const AdminTenants = () => {
  ...
  const handleBrandingUpload = async (
    ...
    try {
      ...
      const folder = field === "logo_url" ? "logos" : "favicons";
      const path = `tenant-branding/${folder}/${Date.now()}_${Math.random().toString(36).slice(2, 6)}.${ext}`;
      const { error } = await supabase.storage.from("assets").upload(path, file, { upsert: true });
      if (error) { toast.error("Upload failed: " + error.message); return; }
      const { data: urlData } = supabase.storage.from("assets").getPublicUrl(path);
      updateSetting(field, urlData.publicUrl);
      toast.success(field === "logo_url" ? "Logo uploaded!" : "Favicon uploaded!");
    } catch (err: any) {
  ...
  const [smtpDialog, setSmtpDialog] = useState<Tenant | null>(null);
  ...
  const openEdit = (t: Tenant) => {
    ...
    setForm({
      ...
      allowed_products: (t as any).allowed_products || ["flights", "hotels", "tours"],
  ...
  const provisionDomain = async (tenantId: string, customDomain: string) => {
    ...
    try {
      ...
      toast.success(`Domain ${data.domain} provisioned!`);
      if (data.instructions) toast.info(data.instructions, { duration: 10000 });
    } catch (e: any) {
      toast.error(`Domain provisioning failed: ${e.message}`);
    } finally {
      setProvisioningDomain(false);
    }
  };

  const checkDomainStatus = async (tenantId: string) => {
    try {
      const { data, error } = await supabase.functions.invoke("cloudflare-domain", {
        body: { action: "check_status", tenant_id: tenantId },
      });
      if (error) throw error;
      toast.success(`Domain status: ${data?.cf_status || "unknown"}`);
      fetchTenants();
    } catch (e: any) {
  ...
  const handleSave = async () => {
    ...
    if (!form.domain && !editing) {
      toast.error("Custom domain is required");
      return;
    }
    setSaving(true);

    const effectiveDomain = form.domain.toLowerCase().trim();
    ...
    if (editing) {
      ...
      const result = await supabase.from("tenants").insert(payload).select("id").single();
      error = result.error;
    ...
    if (error) {
      ...
      // Auto-provision domain on Cloudflare
      if (tenantId && !editing) {
        await provisionDomain(tenantId, form.domain);
      }
      setDialogOpen(false);
      fetchTenants();
    }
  ...
  return (
    <AdminLayout>
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className="text-xs font-mono">
                              {t.settings?.billing_currency || "USD"}
                            </Badge>
                            {(t as any).cf_domain_status && (
                              <Badge variant={(t as any).cf_domain_status === "active" ? "default" : "secondary"} className="text-[10px]">
                                <Cloud className="w-3 h-3 mr-1" />
                                {(t as any).cf_domain_status}
                              </Badge>
                            )}
                            <Badge variant="outline" className="text-[10px]">Custom Domain</Badge>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                            {((t as any).allowed_products || ["flights","hotels","tours"]).length < 3 ? (
                              ((t as any).allowed_products || []).map((p: string) => (
                                <Badge key={p} variant="default" className="text-[10px] px-1.5 py-0 capitalize">{p}</Badge>
                              ))
                            ) : (
                            ...
                            {groupName ? (
                              ...
                              <span className="text-xs text-muted-foreground italic">No provider group</span>
                            )}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm" onClick={() => checkDomainStatus(t.id)} title="Check Domain Status">
                          <RefreshCw className="w-4 h-4" />
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => openApiKeyDialog(t)} title="API Keys">
                          <Key className="w-4 h-4" />
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => setSmtpDialog(t)} title="Email / SMTP">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="travel.clientsite.com"
                className="mt-1 font-mono"
              />
              <p className="text-xs text-muted-foreground mt-1">Tenant must CNAME their domain to <span className="font-mono">custom.travelvela.com</span></p>
            </div>
            <div>
            ...
            <div className="border-t border-border pt-4">
              <p className="font-semibold text-sm mb-1">Primary Billing Currency</p>
              <p className="text-xs text-muted-foreground mb-3">
                All settlements and wallet transactions for this tenant will be denominated in this currency.
              </p>
              <Select
                value={form.settings.billing_currency || "USD"}
                onValueChange={(v) => updateSetting("billing_currency", v)}
              >
            ...
            <div className="border-t border-border pt-4">
              <p className="font-semibold text-sm mb-1">Product Access Level</p>
              <p className="text-xs text-muted-foreground mb-3">
                Control which products this tenant can display and sell on their white-label site.
              </p>
                ...
                {ALL_PRODUCTS.map((product) => (
                  <div key={product.key} className="flex items-center gap-2">
                    <Checkbox
                      ...
                      checked={form.allowed_products.includes(product.key)}
                      onCheckedChange={(checked) => {
                        setForm((prev) => ({
                          ...prev,
                          allowed_products: checked
                            ? [...prev.allowed_products, product.key]
                            : prev.allowed_products.filter((p) => p !== product.key),
                        }));
                      }}
                    />
                    <Label htmlFor={`product-${product.key}`} className="text-sm cursor-pointer">{product.label}</Label>
                  </div>
                ))}
              </div>
              {form.allowed_products.length === 0 && (
                <p className="text-xs text-destructive mt-2">⚠ At least one product should be enabled</p>
              )}
            </div>
            ...
            <div className="border-t border-border pt-4">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    {form.settings.logo_url && (
                      <img src={form.settings.logo_url} alt="Logo" className="h-9 w-auto rounded border border-border object-contain bg-muted" />
                    )}
                    <Input value={form.settings.logo_url} onChange={(e) => updateSetting("logo_url", e.target.value)} placeholder="https://..." className="flex-1" />
                    <label className="cursor-pointer">
                      ...
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                        ...
                        if (f) handleBrandingUpload(f, "logo_url", setUploadingLogo);
                        e.target.value = "";
                      }} />
                    </label>
                  </div>
                ...
                <div>
                  <Label>Favicon</Label>
                  <div className="flex items-center gap-2 mt-1">
                    {form.settings.favicon_url && (
                      <img src={form.settings.favicon_url} alt="Favicon" className="h-7 w-7 rounded border border-border object-contain bg-muted" />
                    )}
                    <Input value={form.settings.favicon_url} onChange={(e) => updateSetting("favicon_url", e.target.value)} placeholder="https://..." className="flex-1" />
                    <label className="cursor-pointer">
                      <Button type="button" size="sm" variant="outline" className="gap-1.5" asChild>
                        <span>
                      ...
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                        const f = e.target.files?.[0];
                        if (f) handleBrandingUpload(f, "favicon_url", setUploadingFavicon);
                        e.target.value = "";
                      }} />
                    </label>
                  </div>
            ...
            {/* Website Content */}
            <div className="border-t border-border pt-4">
              <p className="font-semibold text-sm mb-3">Website Content</p>
              <div className="space-y-3">
                <div><Label className="text-xs">Tagline</Label><Input value={form.settings.tagline || ""} onChange={(e) => updateSetting("tagline", e.target.value)} placeholder="Your travel partner" className="mt-1" /></div>
                <div><Label className="text-xs">Hero Heading</Label><Input value={form.settings.hero_heading || ""} onChange={(e) => updateSetting("hero_heading", e.target.value)} placeholder="Explore the World" className="mt-1" /></div>
                <div><Label className="text-xs">Hero Subheading</Label><Input value={form.settings.hero_subheading || ""} onChange={(e) => updateSetting("hero_subheading", e.target.value)} placeholder="Find the best deals on flights, hotels & tours" className="mt-1" /></div>
                <div><Label className="text-xs">Footer Text</Label><Input value={form.settings.footer_text || ""} onChange={(e) => updateSetting("footer_text", e.target.value)} placeholder="© 2026 Your Brand. All rights reserved." className="mt-1" /></div>
              </div>
            </div>

            {/* SEO */}
            <div className="border-t border-border pt-4">
              <p className="font-semibold text-sm mb-3">SEO Settings</p>
              <div className="space-y-3">
                <div><Label className="text-xs">Meta Title</Label><Input value={form.settings.meta_title || ""} onChange={(e) => updateSetting("meta_title", e.target.value)} placeholder="Best Travel Deals — Your Brand" className="mt-1" /></div>
                <div><Label className="text-xs">Meta Description</Label><Input value={form.settings.meta_description || ""} onChange={(e) => updateSetting("meta_description", e.target.value)} placeholder="Book flights, hotels & tours at the best prices." className="mt-1" /></div>
              </div>
            </div>

            {/* Contact Info */}
            <div className="border-t border-border pt-4">
              <p className="font-semibold text-sm mb-3">Contact Information</p>
              <div className="space-y-3">
                <div><Label className="text-xs">Contact Email</Label><Input value={form.settings.contact_email || ""} onChange={(e) => updateSetting("contact_email", e.target.value)} placeholder="info@yourbrand.com" className="mt-1" /></div>
                <div><Label className="text-xs">Contact Phone</Label><Input value={form.settings.contact_phone || ""} onChange={(e) => updateSetting("contact_phone", e.target.value)} placeholder="+880 1234 567890" className="mt-1" /></div>
              </div>
            </div>

            {/* Social Links */}
            <div className="border-t border-border pt-4">
              <p className="font-semibold text-sm mb-3">Social Media Links</p>
              <div className="space-y-3">
                <div><Label className="text-xs">Facebook</Label><Input value={form.settings.social_facebook || ""} onChange={(e) => updateSetting("social_facebook", e.target.value)} placeholder="https://facebook.com/yourbrand" className="mt-1" /></div>
                <div><Label className="text-xs">Instagram</Label><Input value={form.settings.social_instagram || ""} onChange={(e) => updateSetting("social_instagram", e.target.value)} placeholder="https://instagram.com/yourbrand" className="mt-1" /></div>
                <div><Label className="text-xs">Twitter / X</Label><Input value={form.settings.social_twitter || ""} onChange={(e) => updateSetting("social_twitter", e.target.value)} placeholder="https://x.com/yourbrand" className="mt-1" /></div>
                <div><Label className="text-xs">WhatsApp</Label><Input value={form.settings.social_whatsapp || ""} onChange={(e) => updateSetting("social_whatsapp", e.target.value)} placeholder="+880 1234 567890" className="mt-1" /></div>
              </div>
            </div>
          </div>
          <DialogFooter>
      ...
      {/* SMTP Config Dialog */}
      <Dialog open={!!smtpDialog} onOpenChange={(open) => !open && setSmtpDialog(null)}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Mail className="w-5 h-5 text-primary" />
              Email / SMTP — {smtpDialog?.name}
            </DialogTitle>
          </DialogHeader>
          {smtpDialog && (
            <SmtpConfigCard tenantId={smtpDialog.id} tenantName={smtpDialog.name} />
          )}
        </DialogContent>
      </Dialog>
    </AdminLayout>
