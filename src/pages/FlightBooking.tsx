...
const FlightBooking = () => {
  ...
  useEffect(() => {
    ...
    // Check sessionStorage for AI trip planner flights
    const fromAi = searchParams.get("fromAi") === "true";
    if (fromAi) {
      const stored = sessionStorage.getItem('ai_flight_booking');
      if (stored) {
        try {
          const parsed = JSON.parse(stored);
          setFlight(parsed as Flight);
          setPageLoading(false);
          sessionStorage.removeItem('ai_flight_booking');
          return;
        } catch {}
      }
    }
  ...
  const verifyPrice = async () => {
    ...
    if (flight?.source === "travelport" && flight.segments?.length) {
      ...
      try {
        ...
        const { data, error } = await supabase.functions.invoke("unified-flight-search", {
          body: maskBodyForWire({ action: "price", source: "travelport", segments: flight.segments, adults: adultCount, children: childCount, infants: infantCount, cabinClass: flight.class, studentFare }),
        });
    ...
    if (flight?.source === "amadeus" && (flight as any).amadeusRawOffer) {
      ...
      try {
        const { data, error } = await supabase.functions.invoke("unified-flight-search", {
          body: maskBodyForWire({ action: "price", source: "amadeus", rawOffer: (flight as any).amadeusRawOffer }),
        });
  ...
  const reverifyTripjackFare = async (): Promise<{ ok: boolean; newTotalFare?: number }> => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("unified-flight-search", {
        body: maskBodyForWire({ action: "price", source: "tripjack", priceIds: [priceId], targetCurrency: displayCurrency }),
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isTravelVela = false; // TravelVela removed
    ...
    if (isApiSourced) {
      try {
        ...
        if (isTravelport) {
          ...
          const result = await supabase.functions.invoke("unified-flight-search", {
            body: maskBodyForWire({
              action: "book",
              source: "travelport",
