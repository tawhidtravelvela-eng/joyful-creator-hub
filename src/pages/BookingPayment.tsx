...
import { processBkashPayment, executeBkashPayment, processAlipayPayment, processAirwallexPayment, confirmAirwallexPayment, updateBookingStatus } from "@/utils/bookingService";
...
const BookingPayment = () => {
  ...
  const bookedCur = booking.booked_currency || currency;
  const fmtTotal = (amt: number) => {
    const sym = CURRENCY_SYMBOLS[bookedCur] || bookedCur + " ";
  ...
  const handlePay = async () => {
    ...
    if (selectedPayment === "alipay") {
      try {
        const alipayResult = await processAlipayPayment(total, booking.booking_id);
        if (!alipayResult.success) {
          toast.error(alipayResult.error || "Alipay payment failed");
          setPaying(false);
          return;
        }
        if (alipayResult.payURL) {
          sessionStorage.setItem("alipay_trade_no", alipayResult.outTradeNo || "");
          sessionStorage.setItem("alipay_booking_db_id", booking.id);
          window.location.href = alipayResult.payURL;
          return;
        }
      } catch {
        toast.error("Alipay payment failed.");
        setPaying(false);
        return;
      }
    }

    if (selectedPayment === "airwallex") {
      try {
        const returnUrl = `${window.location.origin}/booking/confirmation`;
        const awxResult = await processAirwallexPayment(total, currency, booking.booking_id, returnUrl);
        if (!awxResult.success) {
          toast.error(awxResult.error || "Airwallex payment initiation failed");
          setPaying(false);
          return;
        }
        // Store intent info and redirect to Airwallex hosted page
        sessionStorage.setItem("airwallex_intent_id", awxResult.intentId || "");
        sessionStorage.setItem("airwallex_booking_db_id", booking.id);

        // Use Airwallex hosted payment page
        const env = "prod"; // Production environment
        const hostedUrl = `https://checkout.airwallex.com/checkout?intent_id=${awxResult.intentId}&client_secret=${awxResult.clientSecret}&currency=${currency}`;
        window.location.href = hostedUrl;
        return;
      } catch {
        toast.error("Airwallex payment failed.");
        setPaying(false);
        return;
      }
    }
      ...
      <div className="container mx-auto px-4 py-16 max-w-2xl">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-full bg-warning/50/10 flex items-center justify-center mx-auto mb-4">
              <CreditCard className="w-8 h-8 text-warning0" />
            </div>
          ...
          <Card className="mb-6">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Badge className="bg-warning/50/10 text-warning border border-warning/15">Needs Payment</Badge>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="border-t border-border pt-3 flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-primary">{fmtTotal(total)}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Button className="w-full h-12 text-base font-semibold" onClick={handlePay} disabled={paying}>
            ...
            {paying ? "Processing..." : `Pay ${fmtTotal(total)}`}
          </Button>
        </motion.div>
      </div>
    </Layout>
