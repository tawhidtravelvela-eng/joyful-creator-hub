...
import { Globe, Key, Loader2, Plus, Building2, ExternalLink } from "lucide-react";
          ...
          {requests.map((r) => (
            <Card key={r.id}>
              <CardContent className="py-4">
                ...
                {r.status === "approved" && r.assigned_tenant_name && (
                  <div className="mt-3 p-3 bg-success/5 dark:bg-green-950/30 border border-success/15 dark:border-success rounded-md">
                    <div className="flex items-center gap-2 text-sm">
                      <Building2 className="h-4 w-4 text-success" />
                      <span className="font-medium text-success dark:text-success">Tenant Assigned: {r.assigned_tenant_name}</span>
                    </div>
                    <p className="text-xs text-success dark:text-success mt-1">
                      Your white-label site is being set up. You can configure it from your admin panel.
                    </p>
                  </div>
                )}
