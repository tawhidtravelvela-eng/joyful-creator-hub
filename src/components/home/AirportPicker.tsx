...
async function loadGeoAndPopularity() {
  ...
  geoDataPromise = (async () => {
    try {
      const { detectCountry } = await import("@/utils/geolocation");
      // Load popular routes and geolocation in parallel
      const [routesResult, geoCountry] = await Promise.all([
      ...
      // Process geolocation
      userCountry = geoCountry.code === "US" ? "United States" : geoCountry.code === "GB" ? "United Kingdom" : geoCountry.code === "AE" ? "United Arab Emirates" : geoCountry.name;
    } catch { /* ignore */ }
    geoDataLoaded = true;
  })();
...
function getSmartDefaults(): Airport[] {
  ...
  // Score each airport — priority: Recent → Geo (user country) → Popularity
  const scored = all.map(airport => {
    let score = 0;

    // 1. Recent selections — highest priority (10000+ band)
    const recentIdx = recentCodes.indexOf(airport.code);
    if (recentIdx >= 0) score += 10000 - recentIdx * 100;

    // 2. Same country as user — second priority (1000+ band, always beats popularity)
    const sameCountry = userCountry && airport.country.toLowerCase().includes(userCountry.toLowerCase());
    if (sameCountry) {
      score += 1000;
      // Within same-country, rank by popularity
      if (popularityScores?.has(airport.code)) {
        score += Math.min(popularityScores.get(airport.code)! * 2, 500);
      }
    } else if (popularityScores?.has(airport.code)) {
      // 3. Global popularity for non-local airports (capped under 1000)
      score += Math.min(popularityScores.get(airport.code)! * 2, 900);
    }
          ...
          {showingSmart && results.length > 0 && (
            <div className="flex items-center gap-1.5 px-3 py-1.5">
              ...
              <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                {hasRecents ? "Recent & Nearby" : userCountry ? "Popular Near You" : "Popular Airports"}
              </span>
            </div>
          )}
