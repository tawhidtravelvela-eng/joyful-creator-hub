...
export default function AncillarySection({
  ...
  const fetchTripjackSsr = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("unified-flight-search", {
        body: maskBodyForWire({ action: "ancillaries", source: "tripjack", bookingId, type: "all", targetCurrency: displayCurrency }),
      });
  ...
  const fetchTravelportSsr = async () => {
    ...
    // Fetch all ancillaries via unified endpoint (handles parallel seatmap + ancillaries internally)
    const { data: ancData, error: ancError } = await supabase.functions.invoke("unified-flight-search", {
      body: maskBodyForWire({ action: "ancillaries", source: "travelport", segments, adults, children, infants, targetCurrency: displayCurrency }),
    });

    if (!ancError && ancData?.success) {
      if (ancData.seatMaps?.length) setSeatMaps(ancData.seatMaps);
      if (ancData.baggageOptions?.length) {
        const mapped: SsrOption[] = ancData.baggageOptions.map((b: any) => ({
      ...
      if (ancData.mealOptions?.length) {
        const mapped: SsrOption[] = ancData.mealOptions.map((m: any) => ({
