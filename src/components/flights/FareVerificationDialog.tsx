...
export interface PaxBreakdown {
  label: string; // e.g. "Adult", "Child", "Infant"
  count: number;
  unitPrice: string; // formatted price per person
  subtotal: string; // formatted total for this type
}

export type FareVerificationState =
  | { status: "verifying" }
  | { status: "verified"; onProceed: () => void }
  | { status: "price_changed"; type: "increased" | "decreased"; oldPrice: string; newPrice: string; diff: string; paxBreakdown?: PaxBreakdown[]; onProceed: () => void; onSearchAgain?: () => void }
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
        <motion.div
          ...
          className="bg-card rounded-2xl shadow-2xl border border-success/50/20 max-w-sm w-full mx-4 overflow-hidden"
        >
          ...
          <div className="p-8 text-center">
            <motion.div
              ...
              className="mx-auto w-16 h-16 rounded-full bg-success/50/10 flex items-center justify-center mb-4"
            >
              <CheckCircle2 className="w-9 h-9 text-success0" />
            </motion.div>
      ...
      <AlertDialog open onOpenChange={() => onClose()}>
        <AlertDialogContent className="max-w-[420px] p-0 gap-0 border-0 overflow-hidden rounded-2xl shadow-2xl">
          ...
          <div className={cn(
            "h-1.5",
            isIncrease
              ? "bg-gradient-to-r from-warning/40 via-warning/50 to-warning/50"
              : "bg-gradient-to-r from-success/40 via-success/50 to-teal-500"
          )} />
            ...
            <AlertDialogHeader className="text-center sm:text-center space-y-4">
              ...
              >
                <div className={cn(
                  ...
                  isIncrease ? "bg-warning/50/8" : "bg-success/50/8"
                )}>
                  <div className={cn(
                    "absolute inset-1 rounded-full border-2 border-dashed",
                    isIncrease ? "border-warning/30" : "border-success/30"
                  )} />
                  <div className={cn(
                    "w-12 h-12 rounded-full flex items-center justify-center",
                    isIncrease
                      ? "bg-gradient-to-br from-warning/20 to-warning/50/20"
                      : "bg-gradient-to-br from-success/20 to-teal-500/20"
                  )}>
                    {isIncrease ? (
                      <AlertTriangle className="w-6 h-6 text-warning0" />
                    ) : (
                      <TrendingDown className="w-6 h-6 text-success0" />
                    )}
                  </div>
                </div>
                ...
                <motion.div
                  ...
                  className={cn(
                    ...
                    isIncrease
                      ? "bg-warning/50 text-white shadow-lg shadow-warning/50/25"
                      : "bg-success/50 text-white shadow-lg shadow-success/50/25"
                  )}
                >
                ...
                <div className="space-y-4">
                  ...
                  <motion.div
                    ...
                    className={cn(
                      ...
                      isIncrease ? "border-warning/50 dark:border-warning/50/20" : "border-success/50 dark:border-success/50/20"
                    )}
                  >
                    <div className="flex items-stretch">
                      ...
                      <div className="flex items-center justify-center w-12 relative">
                        ...
                        <motion.div
                          ...
                          className={cn(
                            ...
                            isIncrease
                              ? "bg-warning/50 shadow-warning/50/20"
                              : "bg-success/50 shadow-success/50/20"
                          )}
                        >
                      ...
                      <div className={cn(
                        ...
                        isIncrease ? "bg-warning/50 dark:bg-warning/50/5" : "bg-success/50 dark:bg-success/50/5"
                      )}>
                        <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground/70 font-semibold mb-2">
                          Updated
                        </p>
                        <p className={cn(
                          ...
                          isIncrease ? "text-warning dark:text-warning" : "text-success dark:text-success"
                        )}>
                          {state.newPrice}
                        </p>
                      </div>
                    ...
                    <div className={cn(
                      ...
                      isIncrease
                        ? "bg-warning/80 text-warning dark:bg-warning/50/10 dark:text-warning"
                        : "bg-success/80 text-success dark:bg-success/50/10 dark:text-success"
                    )}>
                      {isIncrease ? (
                        <>
                  ...
                  {state.paxBreakdown && state.paxBreakdown.length > 0 && (
                    <motion.div
                      initial={{ opacity: 0, y: 8 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      <div className="px-3 py-2 border-b border-border/40">
                        <p className="text-[10px] uppercase tracking-[0.12em] font-bold text-muted-foreground/70">
                          Price Breakdown
                        </p>
                      </div>
                      <div className="divide-y divide-border/30">
                        {state.paxBreakdown.map((pax, i) => (
                          <div key={i} className="flex items-center justify-between px-3 py-2">
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-medium text-foreground">{pax.label}</span>
                              <span className="text-[10px] text-muted-foreground">× {pax.count}</span>
                              <span className="text-[10px] text-muted-foreground/60">({pax.unitPrice} each)</span>
                            </div>
                            <span className="text-xs font-bold text-foreground">{pax.subtotal}</span>
                          </div>
                        ))}
                      </div>
                    </motion.div>
            ...
            <div className="flex flex-col sm:flex-row gap-2.5">
              ...
              <AlertDialogAction
                ...
                className={cn(
                  ...
                  isIncrease
                    ? "bg-gradient-to-r from-warning/50 to-warning/50 hover:from-warning hover:to-warning shadow-warning/50/20"
                    : "bg-gradient-to-r from-success/50 to-teal-500 hover:from-success hover:to-teal-600 shadow-success/50/20"
                )}
              >
