...
import { ChevronDown, ChevronUp, Plane, Luggage, Clock, Shield, AlertCircle, Loader2, X, MapPin, Sparkles } from "lucide-react";
...
/** Resolve IATA code to "City (CODE)" format */
function resolveCity(code: string): string {
  if (!code) return "???";
  const ap = airports.find(a => a.code === code);
  return ap?.city ? `${ap.city} (${code})` : code;
}

const FlightDetailsPanel = ({ flight, airlineName, hasReturn, adults = 1, children = 0, infants = 0, studentFare = false, defaultOpen = false }: FlightDetailsPanelProps) => {
  ...
  const [detailsFetchedFor, setDetailsFetchedFor] = useState<string | null>(null);
   ...
   // Fare rules data (used for policy penalty parsing)
  ...
  useEffect(() => {
    ...
    if (flight.source === "travelport") {
      // Travelport: use unified-flight-search price action for backend conversion
      supabase.functions
        .invoke("unified-flight-search", {
          body: maskBodyForWire({
            action: "price",
            source: "travelport",
      ...
      supabase.functions
          .invoke("unified-flight-search", {
            body: maskBodyForWire({ action: "fareRules", source: "tripjack", priceId: tjPriceId, flowType: "SEARCH", targetCurrency: displayCurrency }),
          })
          .then(({ data }) => {
  ...
  // When used inline (defaultOpen), the parent already controls open/close,
  // so we hide the redundant in-panel toggle and render a stylish header strip instead.
  const inlineMode = defaultOpen;

  return (
    <>
    <div className={inlineMode ? "" : "border-t border-border/60"}>
      {!inlineMode && (
        <button
      ...
      {inlineMode && (
        <div className="relative overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-3 px-4 md:px-6 py-3 bg-gradient-to-r from-primary/[0.06] via-primary/[0.03] to-transparent border-b border-primary/10">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="h-7 w-7 rounded-lg bg-primary/12 flex items-center justify-center ring-1 ring-primary/20">
                <Plane className="w-3.5 h-3.5 text-primary" />
              </div>
              <div className="min-w-0">
                <p className="text-[12px] font-bold text-foreground tracking-tight leading-none">Flight Details</p>
                <p className="text-[10.5px] text-muted-foreground mt-1 leading-none">
                  Route, baggage, fare rules &amp; price breakdown
                </p>
              </div>
            </div>
            <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-semibold text-primary/80 bg-primary/10 px-2 py-1 rounded-full">
              <Sparkles className="w-2.5 h-2.5" /> Live verified
            </span>
          </div>
        </div>
      )}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            initial={inlineMode ? false : { height: 0, opacity: 0 }}
          ...
          >
            <div className={cn(
              "px-3 md:px-5 pb-4 md:pb-5",
              inlineMode
                ? "pt-3 md:pt-4 bg-gradient-to-b from-muted/20 to-transparent"
                : "border-t border-border/40"
            )}>
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 md:gap-5 pt-3 md:pt-4">
                ...
                <div className="space-y-3 md:space-y-4 min-w-0">
                  ...
                  {segments ? (<>{(() => {
                    // Detect multi-city by checking segment groups
                    const groupSet = new Set(segments.map((s: any) => String(s.group ?? 0)));
                    const isMultiLeg = groupSet.size > 1;
                    const groupCount = groupSet.size;
                    let lastGroup = "-1";

                    const getLegLabel = (groupIdx: string) => {
                      if (groupCount === 2) return groupIdx === "0" ? "Outbound" : "Return";
                      if (groupCount === 1) return "Outbound";
                      return `Leg ${Number(groupIdx) + 1}`;
                    };

                    const getLegIcon = (groupIdx: string) => {
                      if (groupCount === 2 && groupIdx === "1") return "↩";
                      return "→";
                    };
                    ...
                    const formatGapDuration = (mins: number) => {
                      ...
                      if (days > 0) {
                        const nights = Math.max(days - (hours > 12 ? 0 : 1), days);
                        return { days, nights: Math.max(nights, 1), hours: hours, text: days >= 1 ? `${days} day${days > 1 ? 's' : ''}` : `${hours}h` };
                      }
                      return { days: 0, nights: 0, hours, text: `${hours}h ${mins % 60}m` };
                    };
                    ...
                    const layoverMins = nextSeg && seg.arrival && nextSeg.departure && seg.arrival.includes("T") && nextSeg.departure.includes("T")
                      ? Math.round((new Date(nextSeg.departure).getTime() - new Date(seg.arrival).getTime()) / 60000)
                      : 0;
                    const layoverDur = layoverMins > 0
                      ? (() => { const h = Math.floor(layoverMins / 60); const m = layoverMins % 60; return h > 0 ? `${h}h ${m}m` : `${m}m`; })()
                      : null;
                    
                    const currentGroup = String(seg.group ?? 0);
                    const showLegHeader = isMultiLeg && currentGroup !== lastGroup;
                    const isLegChange = nextSeg && String(nextSeg.group ?? 0) !== currentGroup;
                    lastGroup = currentGroup;

                    const lastSegOfGroup = segments.filter((s: any) => String(s.group ?? 0) === currentGroup).pop();
                    const legLabel = getLegLabel(currentGroup);
                    const legRoute = `${seg.origin} ${getLegIcon(currentGroup)} ${lastSegOfGroup?.destination || ''}`;
                    
                    return (
                    <div key={idx}>
                      {showLegHeader && (
                        <div className={`flex items-center gap-3 mb-3 ${currentGroup !== "0" ? 'mt-2' : 'mt-0'}`}>
                          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider ${
                            ...
                            <Plane className={`w-3 h-3 ${currentGroup === "1" ? 'rotate-180' : ''}`} />
                            {legLabel}
                          </div>
                          <span className="text-[11px] font-medium text-muted-foreground">{legRoute}</span>
                          <div className="flex-1 border-t border-border/40" />
                        </div>
                      )}
                      <div className="bg-card rounded-xl p-4 md:p-5 border border-border/40 shadow-sm">
                        {/* Airline header row */}
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-muted/60 border border-border/50 flex items-center justify-center overflow-hidden flex-shrink-0">
                              <img src={`https://pics.avs.io/64/64/${seg.carrier || flight.airline}.png`} alt="" className="w-5 h-5 md:w-6 md:h-6 object-contain"
                                onError={(e) => { e.currentTarget.style.display = "none"; e.currentTarget.parentElement!.innerHTML = '<span class="text-[10px] font-bold text-primary">' + (seg.carrier || flight.airline) + '</span>'; }}
                              />
                            </div>
                            <div>
                              ...
                              <p className="text-[10px] md:text-[11px] text-muted-foreground leading-snug mt-0.5">
                                {seg.flightNumber ? <span className="font-medium text-foreground/70">{formatFlightNum(seg.carrier || flight.airline, seg.flightNumber)}</span> : null}
                                {seg.flightNumber ? <span className="mx-1.5 text-border">|</span> : null}
                              ...
                              {seg.operatingCarrier && seg.operatingCarrier !== (seg.carrier || flight.airline) && (
                                <p className="text-[9px] md:text-[10px] text-accent mt-0.5">Operated by {getAirlineName(seg.operatingCarrier)}</p>
                              )}
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 flex-shrink-0">
                            {segDur && (
                              <Badge variant="outline" className="text-[10px] md:text-[11px] h-5 md:h-6 px-2 rounded-full font-medium gap-1">
                                <Clock className="w-2.5 h-2.5 md:w-3 md:h-3" />{segDur}
                              </Badge>
                            )}
                            <Badge variant="outline" className="text-[10px] md:text-[11px] h-5 md:h-6 px-2 rounded-full font-medium">Seg {idx + 1}</Badge>
                          </div>
                        </div>

                        {/* Time & route row */}
                        <div className="flex items-center gap-3 md:gap-5">
                          <div className="text-right min-w-[60px] md:min-w-[70px]">
                            <p className="text-base md:text-lg font-bold text-foreground leading-tight">{fmtTime(seg.departure)}</p>
                            {fmtDate(seg.departure) && <p className="text-[10px] md:text-[11px] text-muted-foreground mt-0.5">{fmtDate(seg.departure)}</p>}
                            <p className="text-[10px] md:text-[11px] font-medium text-muted-foreground mt-0.5">{seg.origin}</p>
                          </div>
                          ...
                          <div className="text-left min-w-[60px] md:min-w-[70px]">
                            <p className="text-base md:text-lg font-bold text-foreground leading-tight">{fmtTime(seg.arrival)}</p>
                            {fmtDate(seg.arrival) && <p className="text-[10px] md:text-[11px] text-muted-foreground mt-0.5">{fmtDate(seg.arrival)}</p>}
                            <p className="text-[10px] md:text-[11px] font-medium text-muted-foreground mt-0.5">{seg.destination}</p>
                          </div>
                        </div>
                      </div>
                      {idx < segments.length - 1 && (
                        isLegChange ? (() => {
                          const gap = layoverMins > 0 ? formatGapDuration(layoverMins) : null;
                          const cityName = resolveCity(seg.destination);
                          return (
                            <div className="relative my-3 md:my-4">
                              {/* Decorative line */}
                              <div className="absolute left-5 top-0 bottom-0 w-px bg-gradient-to-b from-primary/20 via-primary/10 to-accent/20" />
                              <div className="ml-3 flex items-center gap-3 bg-gradient-to-r from-primary/5 via-accent/5 to-transparent rounded-xl px-4 py-3 border border-primary/10">
                                ...
                                <div>
                                  ...
                                  <p className="text-[10px] md:text-[11px] text-muted-foreground mt-0.5">
                                    {gap && gap.days >= 1 
                                      ? `${gap.days} night${gap.days > 1 ? 's' : ''} · ${gap.text}`
                                      : layoverDur ? `${layoverDur} gap` : ''
                                    }
                                  </p>
                                </div>
                              </div>
                          ...
                          <div className="flex items-center gap-2 text-[10px] md:text-[11px] text-accent-foreground font-semibold bg-accent/5 rounded-lg px-3 md:px-4 py-1.5 md:py-2 my-2 md:my-2.5 border border-accent/10">
                            <Clock className="w-3 h-3 md:w-3.5 md:h-3.5 flex-shrink-0" />
                            <span>Layover at {resolveCity(seg.destination)} · {layoverDur || "—"}</span>
                          </div>
                        )
                      )}
                    </div>
                      ...
                      <div className="flex items-center gap-2 md:gap-3">
                        ...
                        <div className="flex-1 flex flex-col items-center gap-0.5">
                          ...
                          <div className="flex items-center gap-1.5 md:gap-2 text-[9px] md:text-[10px] text-muted-foreground">
                            ...
                            <span>·</span>
                            <span className={flight.stops === 0 ? "text-success dark:text-success font-medium" : ""}>{flight.stops === 0 ? "Non-stop" : `${flight.stops} Stop`}</span>
                          </div>
                        </div>
                      ...
                      {detailsLoading ? (
                        ...
                        <>
                          ...
                          {effectiveRefundable === true && (
                            <div className="flex items-center gap-2 bg-success/5 dark:bg-success/10 rounded-xl px-3.5 py-2.5 border border-success/15 dark:border-success/30">
                              <div className="w-6 h-6 rounded-full bg-success/10 dark:bg-success/20 flex items-center justify-center flex-shrink-0">
                                <Shield className="w-3 h-3 text-success dark:text-success" />
                              </div>
                              <span className="text-success dark:text-success text-xs font-semibold">Refundable ticket</span>
                            </div>
                          )}
                          ...
                          {(() => {
                            ...
                            const valColor = (v: string) => cn("font-semibold text-xs", v === "Full fare forfeited" || v === "Non-Refundable" || v === "Not Permitted" ? "text-destructive" : v.includes("Free") ? "text-success dark:text-success" : "text-foreground");
                            ...
                            return (
                              <>
                                ...
                                {effectiveRefundable !== false && effectiveNoShowPenalty && renderPolicyCard(
                                  "No-Show",
                                  <div className="w-5 h-5 rounded-md bg-warning/50/10 flex items-center justify-center"><AlertCircle className="w-2.5 h-2.5 text-warning0" /></div>,
                                  undefined,
