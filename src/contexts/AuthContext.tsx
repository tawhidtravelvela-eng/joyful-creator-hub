...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const checkAdmin = async (userId: string) => {
    try {
      // Fetch role + tenant in parallel
      const [roleRes, tenantRes] = await Promise.all([
        supabase.rpc("has_role", { _user_id: userId, _role: "admin" }),
        supabase.rpc("get_admin_tenant_id", { _user_id: userId }),
      ]);
      const isUserAdmin = !!roleRes.data;
      setIsAdmin(isUserAdmin);
      setAdminTenantId(isUserAdmin ? (tenantRes.data || null) : null);
    } catch (error) {
