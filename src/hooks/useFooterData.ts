...
const defaults: FooterData = { footer: {}, contact: {}, social: {}, header: {} };
    ...
    const load = async () => {
      try {
        ...
        if (tenant) {
          ...
          if (ts.header) result.header = ts.header;
        } else {
          // Global: fetch from api_settings
          const { data: rows } = await supabase
            .from("api_settings")
            .select("provider, settings")
            .in("provider", ["site_footer", "site_contact", "site_social", "site_header"]);
