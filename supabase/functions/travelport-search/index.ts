...
function buildFlightObj(index: number, currency: string, totalPrice: number | null, routeSegments: any[], refundable: boolean, changePenalties: any, cancelPenalties: any, source: string, basePrice?: number | null, taxesAmount?: number | null, baggage?: { cabin?: string; checkin?: string } | null, paxPricing?: Record<string, { base: number; taxes: number; total: number }> | null): any | null {
  ...
  // For round-trip (mixed groups), compute duration from outbound leg only
  const outboundSegs = routeSegments.filter(s => String(s.group) === "0");
  const durationSegs = outboundSegs.length > 0 ? outboundSegs : routeSegments;
  const durFirst = durationSegs[0];
  const durLast = durationSegs[durationSegs.length - 1];
  const depTime = new Date(durFirst.departure).getTime();
  const arrTime = new Date(durLast.arrival).getTime();
  const totalMinutes = Math.round((arrTime - depTime) / 60000);
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;

  // Determine from/to based on outbound leg
  const lastOutbound = outboundSegs.length > 0 ? outboundSegs[outboundSegs.length - 1] : routeSegments[routeSegments.length - 1];
  const stops = Math.max(0, (outboundSegs.length > 0 ? outboundSegs.length : routeSegments.length) - 1);
  ...
  return {
    ...
    to_city: lastOutbound.destination,
    departure: firstSeg.departure,
    arrival: durLast.arrival,
...
function parseFareResults(xmlText: string, criteria: SearchCriteria): any[] {
  ...
  while ((solMatch = solutionRegex.exec(xmlText)) !== null && index < 20) {
    ...
    // Round-trip: combine all selected routes into ONE flight with segments tagged by group
    // Travelport returns a single TotalPrice per AirPricingSolution covering all legs
    if (selectedRoutes.length > 1) {
      const combinedSegments = selectedRoutes.flatMap((route) => route);
      const f = buildFlightObj(index, currency, totalPrice, combinedSegments, refundable, changePenalties, cancelPenalties, "fare", basePrice, taxesAmount, baggage, paxPricing);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.tenantCredentials) {
      ...
      // Load DB settings to determine environment
      ...
      const environment = dbSettings.environment || "production";
      const isPreProd = environment === "test";

      // Pick credentials based on environment
      const secretUsername = isPreProd
        ? (Deno.env.get("TRAVELPORT_PP_USERNAME") || Deno.env.get("TRAVELPORT_USERNAME"))
        : Deno.env.get("TRAVELPORT_USERNAME");
      const secretPassword = isPreProd
        ? (Deno.env.get("TRAVELPORT_PP_PASSWORD") || Deno.env.get("TRAVELPORT_PASSWORD"))
        : Deno.env.get("TRAVELPORT_PASSWORD");
      const secretBranch = isPreProd
        ? (Deno.env.get("TRAVELPORT_PP_TARGET_BRANCH") || Deno.env.get("TRAVELPORT_TARGET_BRANCH"))
        : Deno.env.get("TRAVELPORT_TARGET_BRANCH");

      const defaultEndpoint = isPreProd
      ...
      console.log(`[travelport-search] environment=${environment}, isPreProd=${isPreProd}`);
      ...
      if (secretUsername && secretPassword && secretBranch) {
        settings = {
          ...
          endpoint: dbSettings.endpoint || defaultEndpoint,
        };
      } else {
