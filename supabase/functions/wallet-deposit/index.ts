...
serve(async (req) => {
  ...
  try {
    ...
    const { action, amount, paymentMethod, tenantId, currency, bankAccountId, bankName, transferReference, receiptUrl, trxID, paymentID, id_token } = body;
    ...
    if (action === "bank_deposit") {
      ...
      // Resolve deposit currency:
      // 1. Use explicit currency from caller if present
      // 2. Otherwise fall back to the user's billing_currency on their profile
      // 3. Last resort: USD (so the column is never null)
      ...
      if (!depositCurrency) {
        const { data: prof } = await supabase
          ...
          .eq("user_id", userId)
          .maybeSingle();
        depositCurrency = (prof as any)?.billing_currency || "USD";
      }
      depositCurrency = String(depositCurrency).toUpperCase();
      const descParts = [`Wallet deposit via Bank Transfer (Pending verification)`];
      if (bankName) descParts.push(`Bank: ${bankName}`);
      if (transferReference) descParts.push(`Ref: ${transferReference}`);
    ...
    if (action === "bkash_complete") {
      ...
      const { error } = await supabase.from("wallet_transactions").insert({
        ...
        type: "credit",
        status: "completed",
        currency: executeData.currency || currency || "BDT",
