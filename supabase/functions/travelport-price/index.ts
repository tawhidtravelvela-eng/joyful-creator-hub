...
function parsePriceResponse(xmlText: string, segments?: any[]): any {
  ...
  // Extract per-passenger-type pricing from AirPricingInfo elements
  const paxPricing: Record<string, { base: number; taxes: number; total: number }> = {};
  const pricingInfoRegex2 = /<air:AirPricingInfo[^>]*>([\s\S]*?)<\/air:AirPricingInfo>/g;
  let piMatch;
  while ((piMatch = pricingInfoRegex2.exec(xmlText)) !== null) {
    const block = piMatch[0] + piMatch[1];
    // Find passenger type code
    const paxTypeMatch = block.match(/<com:PassengerType[^>]*Code="([^"]*)"/) 
      || block.match(/PassengerTypeCode="([^"]*)"/);
    if (!paxTypeMatch) continue;
    let paxCode = paxTypeMatch[1]; // e.g., ADT, CNN, INF, STU
    if (paxCode === "CNN") paxCode = "CHD";
    if (paxCode === "STU") paxCode = "ADT";
    if (paxPricing[paxCode]) continue; // already captured this type

    // Extract per-pax prices from the AirPricingInfo tag attributes
    const piAttrs = piMatch[0].match(/<air:AirPricingInfo([^>]*)>/)?.[1] || "";
    const piTotal = piAttrs.match(/TotalPrice="[A-Z]{3}([\d.]+)"/) || piAttrs.match(/ApproximateTotalPrice="[A-Z]{3}([\d.]+)"/);
    const piBase = piAttrs.match(/ApproximateBasePrice="[A-Z]{3}([\d.]+)"/) || piAttrs.match(/BasePrice="[A-Z]{3}([\d.]+)"/);
    const piTax = piAttrs.match(/ApproximateTaxes="[A-Z]{3}([\d.]+)"/) || piAttrs.match(/Taxes="[A-Z]{3}([\d.]+)"/);

    if (piTotal) {
      const t = parseFloat(piTotal[1]);
      const b = piBase ? parseFloat(piBase[1]) : t;
      const tx = piTax ? parseFloat(piTax[1]) : t - b;
      paxPricing[paxCode] = { base: b, taxes: tx, total: t };
    }
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const isPreProd = (dbSettings.environment || "production") === "test";
    const secretUsername = isPreProd ? (Deno.env.get("TRAVELPORT_PP_USERNAME") || Deno.env.get("TRAVELPORT_USERNAME")) : Deno.env.get("TRAVELPORT_USERNAME");
    const secretPassword = isPreProd ? (Deno.env.get("TRAVELPORT_PP_PASSWORD") || Deno.env.get("TRAVELPORT_PASSWORD")) : Deno.env.get("TRAVELPORT_PASSWORD");
    const secretBranch = isPreProd ? (Deno.env.get("TRAVELPORT_PP_TARGET_BRANCH") || Deno.env.get("TRAVELPORT_TARGET_BRANCH")) : Deno.env.get("TRAVELPORT_TARGET_BRANCH");
    const defaultEndpoint = isPreProd ? "https://apac.universal-api.pp.travelport.com/B2BGateway/connect/uAPI/AirService" : "https://apac.universal-api.travelport.com/B2BGateway/connect/uAPI/AirService";
    const settings: TravelportSettings = (secretUsername && secretPassword && secretBranch)
      ? { target_branch: secretBranch, username: secretUsername, password: secretPassword, endpoint: dbSettings.endpoint || defaultEndpoint }
