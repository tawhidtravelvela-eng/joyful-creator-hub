...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Extract per-passenger-type pricing from totalPriceList
    const paxPricing: Record<string, { base: number; taxes: number; total: number }> = {};
    for (const trip of tripInfos) {
      if (!trip?.totalPriceList?.length) continue;
      const fd = trip.totalPriceList[0]?.fd;
      if (!fd) continue;
      const paxMap: Record<string, string> = { ADULT: "ADT", CHILD: "CHD", INFANT: "INF" };
      for (const [key, code] of Object.entries(paxMap)) {
        const fare = fd[key];
        if (!fare?.fC) continue;
        const base = (fare.fC.BF || 0) + (fare.fC.TAF || 0);
        const taxes = (fare.fC.TAF || 0);
        const total = fare.fC.TF || (base + taxes);
        if (total > 0 && !paxPricing[code]) {
          paxPricing[code] = { base: fare.fC.BF || 0, taxes: fare.fC.TAF || 0, total: fare.fC.TF || total };
        }
      }
    }
