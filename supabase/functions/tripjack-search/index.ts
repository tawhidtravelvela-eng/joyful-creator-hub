...
async function getConfig() {
  ...
  return { isActive: data?.is_active ?? false, proxySecret, proxyBase: PROXY_BASE };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { from, to, departDate, returnDate, adults = 1, children = 0, infants = 0, cabinClass = "Economy", directFlight = false, studentFare = false } = body;
    ...
    // Build route infos — single leg only (multi-city is handled by unified calling per-leg)
    ...
    // Parse trips from search result
    const isRoundTrip = !!returnDate;
    
    // For one-way: use ONWARD, RETURN, COMBO
    // For round-trip: use COMBO
    const tripSections = isRoundTrip ? ["COMBO"] : ["ONWARD", "RETURN", "COMBO"];
    ...
    console.log(`[TripjackSearch] Round-trip=${isRoundTrip}, sections processed: ${tripSections.join(",")}, found keys: ${Object.keys(result?.tripInfos || {}).join(",")}`)
    ...
    for (const { key, trips } of allTrips) {
      for (const trip of trips) {
        ...
        // Build normalized segments with group tag for round-trip identification
        const baseSegGroup = key === "RETURN" ? 1 : 0;
        let segGroupAssigner: (seg: TjSegment, idx: number) => number;
        
        if (key === "COMBO" && returnDate) {
          // COMBO = combined round-trip. Split by finding where the destination is reached.
          const destCode = to.toUpperCase();
          let reachedDest = false;
          const groups: number[] = [];
          for (let i = 0; i < segments.length; i++) {
            const segDest = (segments[i].aa?.code || "").toUpperCase();
            if (!reachedDest) {
              groups.push(0);
              if (segDest === destCode) reachedDest = true;
            } else {
              groups.push(1);
            }
          }
          segGroupAssigner = (_seg, idx) => groups[idx] ?? 0;
        } else {
          segGroupAssigner = () => baseSegGroup;
        }
        ...
        const normalizedSegments = segments.map((seg, idx) => ({
          ...
          segmentId: seg.id || "",
          group: segGroupAssigner(seg, idx),
        }));
        ...
        // For COMBO round-trips, use outbound-only metadata for card display
        const outboundNormSegs = normalizedSegments.filter(s => s.group === 0);
        const outFirstSeg = outboundNormSegs[0];
        const outLastSeg = outboundNormSegs[outboundNormSegs.length - 1];
        const isComboRT = key === "COMBO" && returnDate && outboundNormSegs.length < normalizedSegments.length;

        let displayDuration = formatMinutes(totalDuration);
        let displayStops = actualStops;
        let displayDeparture = firstSeg.dt || "";
        let displayArrival = lastSeg.at || "";
        let displayToCity = lastSeg.aa?.code || to;

        if (isComboRT && outFirstSeg && outLastSeg) {
          // Calculate outbound-only duration
          const obSegs = segments.slice(0, outboundNormSegs.length);
          let obDur = 0;
          for (const s of obSegs) {
            obDur += s.duration || 0;
            if (s.cT) obDur += s.cT;
          }
          displayDuration = formatMinutes(obDur);
          displayStops = Math.max(0, outboundNormSegs.length - 1);
          displayDeparture = outFirstSeg.departure || displayDeparture;
          displayArrival = outLastSeg.arrival || displayArrival;
          displayToCity = outLastSeg.to || to;
        }
