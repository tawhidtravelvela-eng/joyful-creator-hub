...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const flights = offers.map((offer: any, idx: number) => {
      ...
      const outFirst = outboundSegs[0] || firstSegment;
      const outLast = outboundSegs.length > 0 ? outboundSegs[outboundSegs.length - 1] : segments[segments.length - 1];
      ...
      return {
        ...
        airline: outFirst?.carrier || "Unknown",
        flightNumber: outFirst?.flightNumber || "",
        from_city: outFirst?.origin || from,
        to_city: outLast?.destination || to,
        departure: outFirst?.departure || "",
        arrival: outLast?.arrival || "",
        duration: offer.itineraries?.[0]?.duration
          ? formatDuration(offer.itineraries[0].duration)
          : "",
        stops: Math.max(0, outboundSegs.length > 0 ? outboundSegs.length - 1 : segments.length - 1),
        price: totalPrice,
        basePrice,
