...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Blog posts (main site only — tenants have their own sitemaps)
    try {
      const res = await fetch(`${supabaseUrl}/rest/v1/blog_posts?status=eq.published&tenant_id=is.null&select=slug,updated_at&order=updated_at.desc`, { headers });
      if (res.ok) {
